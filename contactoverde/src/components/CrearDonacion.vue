<template>
  <html>
    <BasicLayouts>
      <div class="ui inverted segment">
        <h2 class="ui dividing header">Donaciones</h2>
        <h5>Ingresa la información correspondiente de tu donación</h5>
        <div class="ui inverted form">
          <div class="two fields">
            <div class="field">
              <label>Ingresa tu nombre completo</label>
              <input placeholder="First Name" type="text" />
            </div>
            <div class="field">
              <label>Nombre del artículo</label>
              <input placeholder="Last Name" type="text" />
            </div>
          </div>
          <div class="two fields">
            <div class="field">
              <label>Estado del artículo</label>
              <input placeholder="First Name" type="text" />
            </div>
            <div class="field">
              <label>Peso aproximado</label>
              <input placeholder="Last Name" type="text" />
            </div>
          </div>
          <div class="two fields">
            <div class="field">
              <label>Teléfono de contacto</label>
              <input placeholder="First Name" type="text" />
            </div>
            <div class="field">
              <label>Dirección donde se va a recoger</label>
              <input placeholder="Last Name" type="text" />
            </div>
          </div>
          <div class="inline field">
            <div>
              <i class="images outline icon"></i>
              <label>Adjunta una imagen</label>
            </div>
          </div>
          <div class="ui submit olive button">Submit</div>
        </div>
      </div>
    </BasicLayouts>
  </html>
  <div class="ui inverted vertical footer segment">
    <div class="ui container">
      <div class="ui stackable inverted divided equal height stackable grid">
        <div class="three wide column">
          <h4 class="ui inverted header">About</h4>
          <div class="ui inverted link list">
            <a href="#" class="item">Sitemap</a>
            <a href="#" class="item">Contact Us</a>
            <a href="#" class="item">Religious Ceremonies</a>
            <a href="#" class="item">Gazebo Plans</a>
          </div>
        </div>
        <div class="three wide column">
          <h4 class="ui inverted header">Services</h4>
          <div class="ui inverted link list">
            <a href="#" class="item">Banana Pre-Order</a>
            <a href="#" class="item">DNA FAQ</a>
            <a href="#" class="item">How To Access</a>
            <a href="#" class="item">Favorite X-Men</a>
          </div>
        </div>
        <div class="seven wide column">
          <h4 class="ui inverted header">
            Todos los derechos reservados G37 Hackaton UNAL. 2021
          </h4>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
import BasicLayouts from "../layouts/BasicLayouts.vue";

export default {
  name: "Home",
  components: {
    BasicLayouts,
  },
};
</script>

<style scoped>
@import url("https://fonts.googleapis.com/css2?family=Major+Mono+Display&family=Readex+Pro:wght@700&display=swap");

html {
  background-color: white;
  background-size: 100%;
  height: 600px;
  max-width: 1400px;
  margin: 0auto;
}
h1 {
  font-size: 5rem;

  align-text: right;
}
.banner {
  height: 600px;
  color: white;
}
.p {
  font-size: 45rem;
  position: -webkit-sticky;
  position: sticky;
  top: 20px;
}
.footer.segment {
  padding: 5em 0em;
}
</style>
